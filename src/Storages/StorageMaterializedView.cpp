    ...
    : IStorage(table_id_), WithMutableContext(local_context->getGlobalContext())
{
    ...
    if (query.refresh_strategy)
    {
        ...
        refresher = RefreshTask::create(this, getContext(), *query.refresh_strategy, mode >= LoadingStrictnessLevel::ATTACH, refresh_coordinated);
...
void StorageMaterializedView::dropInnerTableIfAny(bool sync, ContextPtr local_context)
{
    ...
    for (StorageID inner_table_id : to_drop)
    {
        ...
        auto table = DatabaseCatalog::instance().tryGetTable(inner_table_id, getContext());
        if (table)
