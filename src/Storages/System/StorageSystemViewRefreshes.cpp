...
ColumnsDescription StorageSystemViewRefreshes::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        {"last_success_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
            "Time when the last successful refresh started. NULL if no successful refreshes happened since server startup or table creation."},
        {"last_success_duration_ms", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "How long the last refresh attempt took."},
        {"last_refresh_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
            "Time when the last refresh attempt finished (if known) or started (if unknown or still running). NULL if no refresh attempts happened since server startup or table creation."},
