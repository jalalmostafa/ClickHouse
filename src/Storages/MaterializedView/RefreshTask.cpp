...
RefreshTask::RefreshTask(
    StorageMaterializedView * view_, ContextPtr context, const DB::ASTRefreshStrategy & strategy, bool attach, bool coordinated)
...
OwnedRefreshTask RefreshTask::create(
    ...
    bool coordinated)
{
    auto task = std::make_shared<RefreshTask>(view, context, strategy, attach, coordinated);
...
void RefreshTask::wait()
{
    auto throw_if_error = [&] {
        ...
        if (!coordination.running_znode_exists && !coordination.root_znode.last_attempt_succeeded)
    ...
    refresh_cv.wait(lock, [&] {
        return state != RefreshState::Running && state != RefreshState::Scheduling &&
            state != RefreshState::RunningOnAnotherReplica && !scheduling.out_of_schedule_refresh_requested && view;
    });
    throw_if_error();
        ...
        while (true)
        {
            ...
            catch (...)
            {
                ...
                else
                {
                    ...
                    message.text = fmt::format("{}: Refresh failed: {}", view->getStorageID().getFullTableName(), error_message);
